...
PrintRemainingHealth()
{
	...
	PrintToChatAll("\x01[SM] Tank had \x05%d\x01 health remaining", g_iLastTankHealth);
}
...
PrintTankDamage()
{
	...
	for (new k; k <= survivor_index; k++)
	{
		...
		for (new i = 1; i <= MaxClients; i++)
		{
    		if (IsClientInGame(i))
    		{
				CPrintToChat(i, "{olive}%d {default}[{green}%i%%{default}] {lightgreen}%N", damage, percent_damage, client);
			}
		}
	}
}
